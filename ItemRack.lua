...
-- this is for the added item bar first click --
...
-- this is for the added itembar swapped item being clicked --
function ItemRack_Menu_OnClick(arg1)
	...
	elseif (UnitAffectingCombat("player") and not ItemRack.Indexes[ItemRack.InvOpen].swappable) or Rack.IsPlayerReallyDead() then
		...
		if not IsShiftKeyDown() then
			...
			if ItemRack.BaggedItems[id].name=="(empty)" then
				Rack.AddToCombatQueue(ItemRack.InvOpen,0)
				Rack.UseQueue[ItemRack.InvOpen] = "queued"
			else
				local _,itemID = Rack.GetItemInfo(ItemRack.BaggedItems[id].bag,ItemRack.BaggedItems[id].slot)
				Rack.AddToCombatQueue(ItemRack.InvOpen,itemID)
				Rack.UseQueue[ItemRack.InvOpen] = "queued"
	...
	elseif IsShiftKeyDown() and ItemRack.InvOpen and cursor_empty() and not SpellIsTargeting() then
		--DEFAULT_CHAT_FRAME:AddMessage(tostring(ItemRack.InvOpen))
		if ItemRack.BaggedItems[id].name=="(empty)" then
			Rack.AddToCombatQueue(ItemRack.InvOpen,0)
			Rack.UseQueue[ItemRack.InvOpen] = "queued"
		else
	...
	elseif ItemRack.InvOpen and cursor_empty() and not SpellIsTargeting() then
		...
		Rack.StopTimer("UseQueue")
		getglobal("ItemRackInv"..ItemRack.InvOpen.."Queue"):Hide()
		getglobal(ItemRack.Indexes[ItemRack.InvOpen].paperdoll_slot.."Queue"):Hide()
		Rack.UseQueue[ItemRack.InvOpen] = nil
		Rack.CombatQueue[ItemRack.InvOpen] = nil
...
Rack = {
	...
	TimerPool = {}, -- timer tables  here ["InvUpdate"]={timer,limit,func,rep,arg}
	...
	UseQueue = {} -- table of items to be swapped after the slot is used
}
...
function Rack.Initialize()
	...
	Rack.CreateTimer("UseQueue",Rack.OnEvent,.25,0,arg)
end

function Rack.OnEvent(overrideslot)
	if event=="ITEM_LOCK_CHANGED" then
		Rack.OnItemLockChanged()
	elseif overrideslot or (event=="PLAYER_REGEN_ENABLED" or event=="PLAYER_UNGHOST" or event=="PLAYER_ALIVE") and (not Rack.IsPlayerReallyDead() and not UnitAffectingCombat("player")) then
	-- player is coming out of combat or being res'ed.  EquipSet the CombatQueue
	if overrideslot then
		local tmr,dur = GetInventoryItemCooldown("player", overrideslot)
		if (dur == 0) then
			Rack.StartTimer("UseQueue",.25,overrideslot)
			return
		end
	end
		local somethingQueued
		for i=0,19 do
			if Rack.CombatQueue[i] and not(Rack.UseQueue[i]=="queued") then
				Rack.UseQueue[i] = nil
				Rack.StopTimer("UseQueue")
			...
			if ((not Rack.UseQueue[i]) or overrideslot == i) then
...
function Rack.CreateTimer(name,func,limit,rep,arg)
	Rack.TimerPool[name] = { func=func, limit=limit, timer=limit, rep=rep, enabled=nil, arg = arg }
end

function Rack.StartTimer(name,timer,arg)
	Rack.TimerPool[name].timer = timer or Rack.TimerPool[name].limit
	Rack.TimerPool[name].enabled = 1
	Rack.TimerPool[name].arg = arg
	RackFrame:Show()
end
...
function Ein_ShieldSwap()
local _,ohaspd = UnitAttackSpeed("player");
	if(not(ohaspd))then
		if(not(GetInventoryItemLink("player",17)))then
			Rack.EquipSet("shield");
		end
	elseif(ohaspd)then
		Rack.EquipSet("shield");
	end
